...
- **Failure guard** — after 3 consecutive failed action rounds the AI stops and explains (rather than thrashing). The runaway backstop is **`max_iterations`** (default **50**) with a "continue?" checkpoint every **`continue_every`** tool calls (default 10). In a **live session, each approved checkpoint *extends* the ceiling**, so a long multi-step task (install + build + test + fix) runs to completion instead of dying mid-build at 50 — keep approving and it keeps going. The hard `max_iterations` ceiling only applies to **unattended** runs (one-shot/piped), where there's no one to approve. The continue prompt offers **`a` = "yes, and don't ask again this task"** so a long trusted run finishes uninterrupted. All three are configurable: `/config set max_iterations 100`. For a single *slow* command, `run_command` takes a `timeout` (max 600s) so it finishes in one step instead of polling. Skipped automatically in one-shot/piped mode
