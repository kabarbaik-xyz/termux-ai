...
![version](https://img.shields.io/badge/version-7.9.7-green)
...
- **Workspace isolation** — sessions anchor to the **workspace root** (nearest `.git`, project manifest, or `CONTEXT.md` above the launch dir — launching from any subdir of a repo lands in the same workspace; nested repos resolve to the nearest). Inside a workspace: auto-resume/`--continue` resumes *only that workspace's* last session (a new project starts fresh — never grabs another project's chat), and `ai -S <name>` slugs are workspace-local (the same name in two repos = two independent sessions; `/load <id>` is the global escape hatch). Outside workspaces (unmarked dirs, home/tmp): exact-cwd sessions resume normally, and the global fallback only applies to sessions that belong to NO workspace — a workspace's session never leaks into an unmarked directory (you get a fresh start plus a dim hint naming where that session lives; `/load <id>` opens it explicitly). `/history` and `/search` are workspace-scoped by default; append `all` (`/history all`, `/search all <q>`) to cross projects. Legacy sessions are backfilled to their workspace root automatically.
...
- **Reasoning-effort control** — per-profile `reasoning_effort` (`low|medium|high`) for gateways with thinking models (NaraRouter/bynara documents it as a no-op on non-reasoning models): `/profile set nararouter.reasoning_effort low` cuts hidden thinking latency dramatically (measured on a bynara reasoner: **33.6s → 16.1s, −52%**). Unset/invalid values and the local Ollama path never send it — other backends' payloads stay byte-identical. A gateway that 400s on the field gets it stripped + one retry + the profile setting cleared (self-healing, like `stream_options`). Use `low` for chat, `medium/high` for hard tasks
- **Free-tier badges** — `/models` on a cloud backend lists the gateway's catalog with **FREE** badges and free models first. Authoritative where possible: bynara's keyed plans API marks the true free tier (surfacing unmarked free models like `agnes-2.5-flash`, `ox-alpha`); other gateways use `-free`/`contributor` name markers plus known aliases (`big-pickle`). The background prefetcher warms the free list too
- **Background catalog prefetch** — on startup (interactive mode, config `model_prefetch`, default on) a daemon thread fetches every cloud backend's model list and re-refreshes it every 10 minutes (staggered, gentle on gateways). `/model <Tab>` is always instant — never a gateway wait — and new/free models appear continuously instead of after a 10-minute cold spot. Piped/one-shot mode skips the thread; failures are silent
- **Model Tab completion for every backend** — `/model <Tab>` lists the ACTIVE backend's catalog live from its `/v1/models` endpoint (local Ollama shows pulled models; cloud gateways show theirs, best-effort 3s). Free-tier models float to the top on gateways that suffix them (`deepseek-v4-flash-free`, `qwen-3.8-max-free`…); Google's `models/` prefix is stripped for clean matching
- **Keep-alive connection pool** — backend requests reuse a persistent HTTPS connection per host instead of opening a fresh TCP+TLS handshake every time (measured on router.bynara.id: median 385ms → 53ms per request exchange, **~330ms saved per call**). In tool loops (5–20 requests per turn) that's seconds back on every cloud backend. Stale connections reopen transparently; the pool caps socket lifetime at 5 minutes
- **Live stream progress + long-doc writing** — buffered gateways can sit silent for 100s+ while generating; the spinner now shows WHAT is streaming (`streaming tool call · 8.4KB` while a document writes, `streaming · 1.2k chars` for answers) and the footer reports measured speed (`↓1.5k · 55 tok/s`). Long documents are written in **sections** (one `write_file` + `write_file(append=true)` per section — first file lands sooner, less lost on drops); a >15KB single write still executes but the model is coached to continue in sections. Stream idle timeout is 240s (`stream_idle_timeout`) for buffered gateways. `/bench` times the active backend (TTFB / tok/s) in ~15s. See *"Why long documents feel stuck"* in the docs: it's usually generation speed, not prefill
- **Token usage & limits (pi-style)** — every backend reports **real** usage: cloud streams get `stream_options.include_usage` (auto-disabled for gateways that reject it, falling back to estimates marked `~`), native Ollama reports its counters, Anthropic reports per-message. After each reply: `↑1.2M ↓340 · 5.0k/128.0k (4%) (r42) (auto)` — input/output, conversation vs the **model's** window, request count, auto-compact armed. The prompt shows context pressure (`| 41.5t | 4%`). Windows are **per model**: cloud from a registry (gpt-4o 128k, gemini 1M, o-series/claude 200k, …), **local = its `num_ctx`** (the phone's real limit — never budgeted, per spec). Auto-compact now fires at `compact_at` (80%) of the effective window instead of a fixed guess. `/tokens [today|7d|all]` shows the per-model table with costs from the price table (`local` for Ollama models); `/status` adds a session usage line
