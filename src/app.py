...
class App:
    ...
    def _chat(self, user_input, title=None):
        ...
        # Document-creation guidance (per-message, NOT in the shared system
        # prompt -- the warm-prefix lock requires message-stability): doc
        # requests make models pick absolute paths outside the cwd sandbox
        # (~/guide.md, /storage/...), doomed at execution. State the rule so
        # the FIRST write uses a relative path (saves a full corrective round).
        _doc_re = re.compile(
            r"\b(guide|guidebook|tutorial|ebook|handbook|manual|report|dokumentasi|panduan)\b", re.IGNORECASE)
        if _doc_re.search(user_input or "") and self.cfg.get("tools_enabled", False):
            msgs[0]["content"] += ("\n\nFILE OUTPUT RULE: when creating a document/guide, write it to a RELATIVE "
                                   "path in the current directory (e.g. 'guide.md'), one section per write "
                                   "with append=true for the rest. Never use absolute or home-directory paths.")
