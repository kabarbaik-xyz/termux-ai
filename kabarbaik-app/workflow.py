...
def stage_recipe(stage_index: int) -> tuple:
    ...
    recipes = {
        ...
        4: (
            ...
            "FULL Budget (Human Resources table, "
            "Infrastructure & Tools, AI operational cost when applicable) "
            "plus Payment Terms (frequency, termin schedule sized to the work "
            "staffed per period, tax basis, update mechanism, payment-due "
            "window) and the *** estimate disclaimer — derive all rates "
            "and amounts per project, market-consistent, recorded as "
            "assumptions. Then "
            "architecture & technology, closing. RFP compliance matrix ONLY "
            "if an RFP exists in the inputs. Budget-conformance verdict vs "
            "any stated budget in docs/discovery/. Output in ENGLISH. "
